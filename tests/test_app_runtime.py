...
async def test_run_cancels_sibling_runtime_when_one_runtime_stops(monkeypatch):
    ...
    async def fake_user_client(received_settings, *, client=None, reaction_history_backfill=None):
    ...
    async def fake_manager_bot(received_settings, *, chat_exporter=None, reaction_history_backfill=None):
        assert received_settings is settings
